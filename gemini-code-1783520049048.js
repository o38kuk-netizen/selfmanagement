...
const T = {
  ink: "#191410",
  inkSoft: "#221B15",
  line: "rgba(239,230,216,0.08)", // より繊細な細線に
...
export default function HabitTracker() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get(weekKey, false);
        if (res && res.value) {
          setWeek(JSON.parse(res.value));
        } else {
          setWeek(emptyWeek());
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.storage.set(weekKey, JSON.stringify(week), false);
  ...
  if (!loaded || !week) {
    return (
      <div style={{ background: T.ink, color: T.ash, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Zen Kaku Gothic New', sans-serif", fontSize: 13, letterSpacing: 0.5 }}>
        整えています
      </div>
    );
  }
  ...
  const adjustTarget = (key, delta) => {
    setWeek((prev) => ({
      ...
      targets: {
        ...prev.targets,
        [key]: { ...prev.targets[key], value: Math.max(0, Math.round((prev.targets[key].value + delta) * 10) / 10) },
      },
    }));
  };
  ...
  const loadHistory = async () => {
    ...
    try {
      const res = await window.storage.list("week:", false);
      const keys = (res?.keys || []).filter((k) => k !== weekKey).sort().reverse().slice(0, 6);
      const items = [];
      for (const k of keys) {
        try {
          ...
          if (r && r.value) {
            ...
            items.push({ key: k, range: formatRange(mon), ...sc });
          }
        } catch (e) { /* skip */ }
      }
      setHistory(items);
  ...
  return (
    <div style={{ background: T.ink, minHeight: "100vh", color: T.paper, paddingBottom: 60 }}>
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "40px 20px 0" }}>
        ...
        <div className="f-display" style={{ fontSize: 28, fontWeight: 700, marginTop: 4, marginBottom: 24, letterSpacing: 0.5 }}>
          今週の火と土
        </div>
        ...
        <div className="f-body" style={{ fontSize: 12.5, color: T.ash, lineHeight: 1.7, marginBottom: 36, letterSpacing: 0.2 }}>
          {summaryText(fire, earth)}
        </div>

        {/* Daily anchors grid */}
        <SectionLabel>日々のアンカー</SectionLabel>
        <div style={{ border: `1px solid ${T.line}`, borderRadius: 8, overflow: "hidden", marginBottom: 32, background: T.inkSoft }}>
          <div style={{ display: "grid", gridTemplateColumns: "68px repeat(7, 1fr)" }}>
            ...
            {FIRE_ROWS.map((row) => (
              <RowCells key={row.key} row={row} week={week} today={today} onToggle={toggleAnchor} activeColor={T.ember} />
            ))}
            <RowCells row={EARTH_ROW} week={week} today={today} onToggle={toggleAnchor} activeColor={T.clay} last />
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
          <TargetRow label="学び（①専門）" data={week.targets.learning} onAdjust={(d) => adjustTarget("learning", d)} accent={T.ember} />
          <TargetRow label="筋トレ" data={week.targets.training} onAdjust={(d) => adjustTarget("training", d)} accent={T.clay} />
          <TargetRow label="サウナ・湯船" data={week.targets.sauna} onAdjust={(d) => adjustTarget("sauna", d)} accent={T.ember} />
        </div>
        ...
        {/* Compass / Mantras */}
        <div style={{ paddingLeft: 16, marginBottom: 40, display: "flex", flexDirection: "column", gap: 8, borderLeft: `1px solid ${T.lineActive}` }}>
          {MANTRAS.map((m) => (
            <div key={m} className="f-body" style={{ fontSize: 12, color: T.ash, lineHeight: 1.6, letterSpacing: 0.2 }}>
              {m}
            </div>
          ))}
        </div>
        ...
        {showHistory && (
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
            {historyLoading && (
              <div className="f-body" style={{ fontSize: 11, color: T.ash, textAlign: "center", padding: "12px 0" }}>
                読み込み中
              </div>
            )}
        ...
        {error && (
          <div className="f-body" style={{ fontSize: 11, color: T.ash, textAlign: "center", marginTop: 12, opacity: 0.6 }}>
            保存に失敗した可能性があります。
          </div>
        )}
      </div>
...
function SectionLabel({ children }) {
  return (
    <div className="f-body" style={{ fontSize: 11, color: T.ash, letterSpacing: 0.8, marginBottom: 12, fontWeight: 500 }}>
      {children}
    </div>
  );
}
          ...
          >
            <button
              ...
              style={{
                width: 18,
                height: 18,
                borderRadius: 3, // 方眼紙のマス目のようなミニマルな四角に
  ...
  return (
    <div style={{ border: `1px solid ${T.line}`, borderRadius: 6, padding: 12, background: T.inkSoft }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ flex: 1, height: 3, background: "rgba(239,230,216,0.05)", overflow: "hidden", borderRadius: 2 }}>
          <div style={{ width: `${pct}%`, height: "100%", background: accent, transition: "width 0.2s ease", opacity: 0.9 }} />
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          <button
            ...
            style={{ width: 24, height: 24, borderRadius: 4, border: `1px solid ${T.line}`, background: "transparent", color: T.ash, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
          >
            <Minus size={11} />
          </button>
          <button
            ...
            className="cell-btn"
            onClick={() => onAdjust(data.step)}
            style={{ width: 24, height: 24, borderRadius: 4, border: `1px solid ${T.line}`, background: "transparent", color: T.paper, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
          >
            <Plus size={11} />
          </button>
        </div>
...
function MiniBar({ value, color, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ width: 24, height: 3, background: "rgba(239,230,216,0.05)", borderRadius: 1, overflow: "hidden" }}>
